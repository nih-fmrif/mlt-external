...
def initialize(data, C, features, Wconstraint, Qconstraint, version):
    # init w, QT
    if C is not None:
        r = C.shape[1]
    else:
    ...
    if version=='random':
        W, QT = random_NMF(data, features+r)
    Q = QT.T
...
def obj_func(M, W, Q, C, beta, mask=None, regularizer=1, mode='model'):
    ...
    betaW = beta
    betaQ = W.shape[0]/Q.shape[0]*beta
...
def find_weights(label):
    ...
    counts=counts/len(label)
    weights=counts[0]*np.ones(len(label))
    if len(vals)>1:
        weights[label==vals[1]]=counts[1]
    return weights
...
def admm(M, k, beta, gamma_flag=(False, None), Q_flag=(False, None), label=None, C=None, rho=2.0, tau=2.0, mask=None,
    ...
    # initialization
    if not Q_flag[0]:
        W, Q, Z, aZ = initialize(M, C, k, Wconstraint, Qconstraint, version='SVD')
    else:
        Q=Q_flag[1]
        W, _, Z, aZ = initialize(M, C, k, Wconstraint, Qconstraint, version='SVD')
...
#     betaW = beta
    ...
    for i in tqdm_iterator:
        ...
        if label is not None:
            ...
            pdb.set_trace()
            model=LR(random_state=0).fit(W,label)
        ...
        if C is not None:
            ...
            B = Z + aZ/rho
        _, W = admm_classo(W.T, Q[:,:k], B.T, betaW, tau, rho, gamma_flag, theta0, theta, label, weights,
